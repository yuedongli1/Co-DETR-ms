...
from common.core.bbox.transforms import box_cxcywh_to_xyxy, generalized_box_iou
...
class SetCriterion(nn.Cell):
    ...
    def __init__(
        ...
        self.matcher = build_assigner(matcher)
    ...
    def _get_matched_target_mindspore(self, outputs, raw_tgt_labels, raw_tgt_boxes, valid_mask):
        ...
        tgt_labels = ms_np.full((bs, num_query + 1), num_class, dtype=ms.float32)  # (bs, num_query)
        ...
        # Boxes
        tgt_boxes = ms_np.full((bs, num_query + 1, 4), 0.0, dtype=ms.float32)
        ...
        tgt_boxes = tgt_boxes[:, :-1, :]

        # valid_mask
        tgt_valids = ops.zeros((bs, num_query + 1), ms.int8)
