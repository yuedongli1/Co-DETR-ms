...
class BboxOverlaps2D(nn.Cell):
    """2D Overlaps (e.g. IoUs, GIoUs) Calculator."""

    def __init__(self, scale=1., dtype=None):
        super(BboxOverlaps2D, self).__init__()
        self.scale = scale
        self.dtype = dtype

    def construct(self, bboxes1, bboxes2, mode='iou', is_aligned=False):
...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    eps = ops.cast(eps, union.dtype)
    eps = ops.stop_gradient(eps).unsqueeze(0)
